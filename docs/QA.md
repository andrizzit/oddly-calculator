...
`OddlyUITests/OddlyUITests.swift` launches the real application for twelve workflows: arithmetic and repeat equals; decimals and percentages; delete and error recovery; portrait scientific tools; history after process relaunch and recall; persistent discoveries without blocked input or moved keys; Calm mode persistence and suppression; clearing history while preserving discoveries; large text in landscape with reachable touch targets; the largest accessibility size in all three secondary panels; landscape arithmetic at that largest size; and press feedback with canceled presses and scrolling from a key. It saves real simulator screenshots for the visual and accessibility cases.
...
To repeat these two targeted checks on a chosen small-phone simulator, substitute its ID below (or run all workflows with `scripts/test-ios.sh`):
...
Remaining release gates: real-device VoiceOver navigation and announcement behavior; haptics and Reduce Motion; Apple Developer membership, distribution signing and App Store Connect setup; TestFlight; and Apple's review/publication. The subsequent signed development installation is recorded below. Automated simulator reachability and label checks cover only part of accessibility validation.
...
On 11 September 2026, all five focused XCUITest cases passed on the iPhone 16 Pro Max simulator running iOS 26.5 after the button-feedback change: arithmetic with repeated equals and fresh entry; portrait scientific tools; history persistence and recall; discoveries and cabinet navigation; and the new `testPressFeedbackPreservesHoldCancellationAndScrolling`. The new regression verifies a held digit activates exactly once in Paper and Midnight, dragging a press outside the key leaves the result unchanged, and a vertical drag starting on a key scrolls the calculator at accessibility5 without entering a digit. Result bundle: `build/UITests-press-feedback.xcresult`; log: `build/qa-press-feedback.log`. The full suite and unchanged arithmetic/model suites were not rerun for this UI change.

A real simulator recording is saved at `build/qa-press-feedback.mp4`. Extracted frames `build/qa-press-paper-held.png` and `build/qa-press-midnight-held.png` visibly show the active digit surface with stationary labels and neighboring keys. The Paper frame captures the short activation highlight after the result changes; the Midnight frame captures the held highlight before release. Resting-state screenshots are included in `build/qa-press-feedback-attachments`. Source review confirms the wrapper still uses a native `Button`, executes its action immediately, introduces no competing gestures, and retains immediate color feedback with Reduce Motion. VoiceOver activation and Reduce Motion were reviewed in code, not newly exercised on a physical device in this pass.

The control room verified the signed Debug build's code signature and installed it through Xcode's device tools on the publisher's iPhone 16 Pro Max running iOS 26.6.2, using the existing Personal Team and bundle identifier. Installation evidence is in `build/feedback-phone-install.json`. The updated unsigned Release archive is `build/Oddly-PressFeedback-Unsigned.xcarchive`; its version, deployment target, privacy manifest, and absence of Debug test switches were checked. Sampled pressed-surface text pairs retain at least 4.65:1 contrast. Installation alone does not verify physical-device haptics or VoiceOver behavior.
