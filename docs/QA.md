...
The control room also verified the final iPad portrait layout: a centered, bounded calculator with readable controls and no clipping, captured in `docs/assets/oddly-ipad-calculator.png`. It built the current production sources into `build/Oddly-1.0.0-Unsigned.xcarchive`. This verifies a native Release archive; it is unsigned and is not a signed-device, TestFlight, or App Store release.

Remaining release gates: signed installation on a physical iPhone; real-device VoiceOver navigation and announcement behavior; haptics and Reduce Motion; the smallest supported phone and the largest accessibility text size in all secondary panels; Apple Developer membership, signing and App Store Connect setup; TestFlight; and Apple's review/publication. Automated simulator reachability and label checks cover only part of accessibility validation.
